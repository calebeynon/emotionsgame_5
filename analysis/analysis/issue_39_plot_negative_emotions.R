...
prepare_plot_data <- function() {
    ...
    # Compute z-scores for anger and fear (not handled by compute_zscores)
    ...
    # Filter to rows with all three measures
...
summarize_by_group <- function(dt, group_col) {
    anger_summary <- dt[, .(
        mean = mean(anger_z), se = sd(anger_z) / sqrt(.N), n = .N
    ), by = group_col]
    anger_summary[, measure := "Anger"]

    fear_summary <- dt[, .(
        mean = mean(fear_z), se = sd(fear_z) / sqrt(.N), n = .N
    ), by = group_col]
    fear_summary[, measure := "Fear"]
    ...
    out <- rbindlist(list(anger_summary, fear_summary, cmp_summary))
