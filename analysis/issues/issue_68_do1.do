...
ciplot contribution, by(period) ylabel(0 (5) 25)  leg(off)

graph export "ciplot_cont_period.png", replace

ciplot contribution if treatment==1, by(period) ylabel(0 (5) 25)  leg(off)

graph export "ciplot_cont_period_t1.png", replace

ciplot contribution if treatment==2, by(period) ylabel(0 (5) 25)  leg(off)

graph export "ciplot_cont_period_t2.png", replace
...
drop if subject_id==103
drop if subject_id==713
drop if subject_id==710
drop if subject_id==411
drop if subject_id==712
drop if subject_id==213
drop if subject_id==303
drop if subject_id==714
drop if subject_id==1012
drop if subject_id==514
drop if subject_id==502
drop if subject_id==410
drop if subject_id==1011
drop if subject_id==809


drop if subject_id==715
drop if subject_id==909
drop if subject_id==912
drop if subject_id==914
drop if subject_id==115
drop if subject_id==1013
drop if subject_id==404
drop if subject_id==910
...
xtabond contribution contmoremax_L1 contlessmax_L1  contmoremed_L1 contlessmed_L1 contmoremin_L1 contlessmin_L1  round1  if treatment==1  , lags(2) twostep maxldep(4) maxlags(4) 
...
xtabond contribution contmore_L1 contless_L1   round1  if treatment==1  , lags(2) twostep maxldep(4) maxlags(4) 
...
outreg2 using "Table DP1", tex dec(3) stats(coef se) symbol(***,**,*) nor2 noni label ctitle(Treatment 1) nonotes
...
xtabond contribution contmore_L1 contless_L1   round1  if treatment==2 , lags(2) twostep maxldep(4) maxlags(4) 
...
tabulate round, summarize(contribution)

ciplot contribution, by(round) ylabel(0 (5) 25)  leg(off)


collapse treatment contribution totalcont , by (subject_id )
