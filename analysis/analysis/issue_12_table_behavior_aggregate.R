#' Behavior Classifications LaTeX Table (Aggregated Across Sessions)
#'
#' Generates a LaTeX table summarizing behavior classifications
#' (promises, liars, suckers) by treatment and segment, aggregated across sessions.
#'
#' Author: Claude
#' Date: 2026-01-25

library(data.table)
library(xtable)

# =====
# File paths
# =====
INPUT_CSV <- "../datastore/derived/behavior_classifications.csv"
OUTPUT_DIR <- "../output/tables"
OUTPUT_FILE <- file.path(OUTPUT_DIR, "behavior_summary_aggregate.tex")

# =====
# Main function
# =====
main <- function() {
    dt <- load_behavior_data(INPUT_CSV)
    summary_dt <- aggregate_behavior_counts(dt)
    latex_table <- create_latex_table(summary_dt)
    ensure_output_dir(OUTPUT_DIR)
    save_latex_table(latex_table, OUTPUT_FILE)
    message("Behavior summary table saved to: ", OUTPUT_FILE)
}

# =====
# Data loading
# =====
load_behavior_data <- function(file_path) {
    fread(file_path)
}

# =====
# Aggregation
# =====
aggregate_behavior_counts <- function(dt) {
    summary_dt <- dt[, .(
        N = .N,
        Promises = sum(made_promise),
        Liars_Strict = sum(is_liar_strict),
        Liars_Lenient = sum(is_liar_lenient),
        Suckers_Strict = sum(is_sucker_strict),
        Suckers_Lenient = sum(is_sucker_lenient)
    ), by = .(Treatment = treatment, Segment = segment)]

    summary_dt <- summary_dt[order(Treatment, Segment)]
    return(summary_dt)
}

# =====
# LaTeX table creation
# =====
create_latex_table <- function(summary_dt) {
    xtable(summary_dt, align = c("l", "c", "l", "r", "r", "r", "r", "r", "r"))
}

# =====
# Output helpers
# =====
ensure_output_dir <- function(dir_path) {
    if (!dir.exists(dir_path)) {
        dir.create(dir_path, recursive = TRUE)
    }
}

save_latex_table <- function(latex_table, file_path) {
    print(latex_table, file = file_path, include.rownames = FALSE,
          booktabs = TRUE, floating = FALSE, hline.after = NULL,
          add.to.row = list(pos = list(-1, 0, nrow(latex_table)),
                           command = c("\\toprule\n", "\\midrule\n", "\\bottomrule\n")),
          sanitize.colnames.function = format_column_names)
}

format_column_names <- function(x) {
    gsub("_", " ", x)
}

# =====
# Execute
# =====
if (sys.nframe() == 0) {
    main()
}
