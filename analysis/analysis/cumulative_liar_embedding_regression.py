"""
Logistic regression: can cumulative-liar-direction embeddings predict contributions?
...
Uses cumulative liar direction vector projections as features.
...
CLIAR_PROJECTIONS_FILE = DERIVED_DIR / 'cumulative_liar_embedding_projections.csv'
SENTIMENT_FILE = DERIVED_DIR / 'sentiment_scores.csv'
OUTPUT_DIR = Path(__file__).parent.parent / 'output' / 'tables'
OUTPUT_FILE = OUTPUT_DIR / 'cumulative_liar_embedding_regression_comparison.tex'
...
def _load_projections() -> pd.DataFrame:
    """Aggregate message-level cumulative liar projections to player-round means."""
    df = pd.read_csv(CLIAR_PROJECTIONS_FILE)
    proj_cols = [c for c in df.columns if c.startswith('proj_cliar_')]
...
def _define_models(dataset: pd.DataFrame) -> dict:
    ...
    proj_cols = [c for c in dataset.columns if c.startswith('proj_cliar_')]
    return {
        'VADER only': dataset[VADER_FEATURES].values,
        'Cum. liar proj. only': dataset[proj_cols].values,
        'Full embedding (PCA 50)': dataset[emb_cols].values,
        'VADER + Cum. liar proj.': dataset[VADER_FEATURES + proj_cols].values,
    }
...
def print_summary(results: pd.DataFrame) -> None:
    ...
    print("\n" + "=" * 70)
    print("CUMULATIVE-LIAR PROJECTION -> PLAYER CONTRIBUTION (5-fold CV)")
    print("=" * 70)
