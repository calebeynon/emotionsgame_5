...
│   ├── raw/                # Raw oTree exports (CSV, chat logs)
│   ├── derived/            # Processed data (contributions.csv, behavior_classifications.csv)
│   └── annotations/        # Video annotation pipeline data
│
├── derived/                # Scripts that compute derived data from raw data
│   ├── classify_promises.py      # LLM-based promise classification
│   ├── classify_behavior.py      # Behavioral classification (cooperator, freerider, etc.)
│   ├── compute_sentiment.py      # VADER sentiment analysis on chat messages
│   ├── merge_regression_data.py  # Combine data sources for regression analysis
│   └── llm_clients/              # API clients for OpenAI, Anthropic, Gemini
│
├── analysis/               # Scripts that produce outputs (plots, tables, regressions)
│   ├── analysis_plots.py         # Visualization and plotting
│   ├── multi_session_analysis.py # Cross-session comparative analysis
│   ├── issue_12_*.R              # R scripts for specific analysis tasks
│   └── *.R                       # Regression and statistical analysis scripts
│
├── annotations/            # Video annotation pipeline for behavioral coding
│   ├── build_edited_data_csv.py  # PageTimes CSV → normalized event data
│   └── generate_annotations.py   # Event data → annotation markers
│
├── output/                 # Generated outputs (gitignored, synced to paper)
...
The `datastore/` symlink points to an external Box folder containing all experimental data. This keeps large data files out of version control while maintaining a consistent path structure.

**Key locations:**
- `datastore/raw/` — Raw oTree data exports and chat logs
- `datastore/derived/` — Processed data files (generated by scripts in `derived/`)
- `datastore/annotations/` — Annotation pipeline inputs and outputs
