"""
Logistic regression: can homogeneity-direction embeddings predict contributions?
...
Uses homogeneity direction vector projections as features.
...
HOMOG_PROJECTIONS_FILE = DERIVED_DIR / 'homogeneity_embedding_projections.csv'
SENTIMENT_FILE = DERIVED_DIR / 'sentiment_scores.csv'
OUTPUT_DIR = Path(__file__).parent.parent / 'output' / 'tables'
OUTPUT_FILE = OUTPUT_DIR / 'homogeneity_embedding_regression_comparison.tex'
...
def _load_projections() -> pd.DataFrame:
    """Aggregate message-level homogeneity projections to player-round means."""
    df = pd.read_csv(HOMOG_PROJECTIONS_FILE)
    proj_cols = [c for c in df.columns if c.startswith('proj_homog_')]
...
def _define_models(dataset: pd.DataFrame) -> dict:
    ...
    proj_cols = [c for c in dataset.columns if c.startswith('proj_homog_')]
    return {
        'VADER only': dataset[VADER_FEATURES].values,
        'Homog. proj. only': dataset[proj_cols].values,
        'Full embedding (PCA 50)': dataset[emb_cols].values,
        'VADER + Homog. proj.': dataset[VADER_FEATURES + proj_cols].values,
    }
...
def print_summary(results: pd.DataFrame) -> None:
    ...
    print("\n" + "=" * 70)
    print("HOMOGENEITY PROJECTION -> PLAYER CONTRIBUTION (5-fold CV)")
    print("=" * 70)
