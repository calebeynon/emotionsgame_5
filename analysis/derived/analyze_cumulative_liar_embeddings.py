"""
Analyze chat embeddings along the cumulative liar vs non-liar axis.

Loads behavior classifications, labels players as cumulative liars (is_liar_20),
...
PROJECTIONS_OUTPUT = DERIVED_DIR / 'cumulative_liar_embedding_projections.csv'

# ANALYSIS CONSTANTS
LIAR_COL = 'cumulative_liar_label'
CLIAR_COL = LIAR_COL
STATE_LIAR = 'liar'
...
def main():
    """Main execution flow."""
    liar_df = load_cumulative_liar_labels()
...
# =====
# Cumulative liar label computation
# =====
def load_cumulative_liar_labels(
    df: pd.DataFrame | None = None,
) -> pd.DataFrame:
    """Load behavior classifications and create cumulative liar labels."""
    if df is None:
        df = pd.read_csv(BEHAVIOR_FILE)
    df[LIAR_COL] = _assign_labels(df['is_liar_20'])
    _print_label_counts(df)
    return df[JOIN_KEYS + [LIAR_COL]]


compute_cumulative_liar_labels = load_cumulative_liar_labels


def _assign_labels(is_liar: pd.Series) -> pd.Series:
    """Map is_liar_20 boolean to liar/non_liar string labels."""
    return is_liar.map(
...
def _print_label_counts(df: pd.DataFrame) -> None:
    """Print distribution of cumulative liar labels."""
    counts = df[LIAR_COL].value_counts()
    print(f"Cumulative liar labels: {dict(counts)}")


def merge_liar_labels(
    meta: pd.DataFrame, liar_df: pd.DataFrame,
) -> pd.DataFrame:
    """LEFT JOIN cumulative liar labels onto embedding metadata."""
    return meta.merge(liar_df, on=JOIN_KEYS, how='left')


merge_cumulative_liar_labels = merge_liar_labels
...
compute_cumulative_liar_centroids = compute_liar_centroids
...
def _build_cross_level_output(results: dict) -> pd.DataFrame:
    ...
    out['proj_cliar_msg_dir_small'] = results['small'][1]
    out['proj_cliar_pr_dir_small'] = results['small'][2]
    out['proj_cliar_msg_dir_large'] = results['large'][1]
    out['proj_cliar_pr_dir_large'] = results['large'][2]
    return out
...
def _build_output(
    ...
    out = meta[id_cols + ['player_state', LIAR_COL]].copy()
    out[f'proj_cliar_pr_dir_{suffix}'] = projections
    return out
