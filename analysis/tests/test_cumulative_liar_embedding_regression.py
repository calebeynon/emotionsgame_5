"""
Tests for cumulative_liar_embedding_regression.py.
...
from cumulative_liar_embedding_regression import (
    _average_metrics,
    _build_latex_table,
    _compute_metrics,
    _define_models,
...
def _make_row(rng, i, n_dims):
    ...
    row = {
        ...
        'proj_cliar_msg_dir_small': rng.randn(),
        'proj_cliar_msg_dir_large': rng.randn(),
        'proj_cliar_pr_dir_small': rng.randn(),
        'proj_cliar_pr_dir_large': rng.randn(),
    }
...
class TestModelPipeline:
    ...
    def test_define_models_names(self):
        ...
        models = _define_models(dataset)
        expected = {
            'VADER only', 'Cum. liar proj. only',
            'Full embedding (PCA 50)', 'VADER + Cum. liar proj.',
        }
    ...
    def test_model_cum_liar_proj_uses_proj_columns(self):
        """Cum. liar proj. model should use proj_cliar columns."""
        dataset = _make_dataset()
        models = _define_models(dataset)
        X_proj = models['Cum. liar proj. only']
...
class TestLatexOutput:
    ...
    def test_contains_model_name(self):
        ...
        results = pd.DataFrame([{
            'model': 'Cum. liar proj. only', 'accuracy': 0.8,
            'auc': 0.9, 'precision': 0.7, 'recall': 0.6, 'f1': 0.65,
        }])
        latex = _build_latex_table(results)
        assert 'Cum. liar proj. only' in latex
