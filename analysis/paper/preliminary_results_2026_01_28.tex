...
\begin{document}
	...
	\begin{titlepage}
		...
		\vspace{2mm}
		\begin{centr}
			{-- \albf{PRELIMINARY DRAFT} -- \\ \al{PLEASE DO NOT CITE OR DISTRIBUTE}}\\[-1mm]{\small Version: 2026-01-28}
		\end{centr}
		\vspace{3mm}
		%%% ABSTRACT %%%
		\begin{abstract}
	...
	\begin{itemize}[nosep]
		\item \textbf{Liar.} A player who made a promise in chat but subsequently contributed below a threshold. Under the \textit{high-threshold} definition, a liar is a promise-maker who contributed $< 20$ (out of 25); under the \textit{low-threshold} definition, a liar contributed $< 5$. The low-threshold definition captures only extreme defection.
		\item \textbf{Sucker.} A player who contributed the maximum (25 points) in a round where a groupmate lied. Under the \textit{high-threshold} definition, a groupmate lied if they promised but contributed $< 20$; under the \textit{low-threshold} definition, the groupmate contributed $< 5$. Because the high-threshold liar definition is easier to meet, it classifies \textit{more} players as suckers.
	\end{itemize}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.85\textwidth]{mean_contribution_by_round.png}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.85\textwidth]{mean_contribution_by_segment.png}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.85\textwidth]{contribution_cdf_by_treatment.png}
	...
	\begin{itemize}[nosep]
		...
		\item \textit{Sucker status} has a large, negative, and highly significant effect. Under the high-threshold definition (groupmate contributed $< 20$ after promising), being a sucker reduces contributions by 5.99 points ($p < 0.01$). Under the low-threshold definition (groupmate contributed $< 5$), the effect is even larger at $-8.02$ points ($p < 0.01$).
	...
	\begin{itemize}[nosep]
		...
		\item \textit{Lying} has a large negative main effect. High-threshold liars (promised but contributed $< 20$) contribute 17.16 points less ($p < 0.01$); low-threshold liars (contributed $< 5$) contribute 22.49 points less ($p < 0.01$).
		\item \textit{Interaction (Sentiment $\times$ Lied):} Under the high-threshold definition, the interaction is small and insignificant ($-0.23$, $p > 0.10$), suggesting sentiment remains equally predictive for liars and non-liars. Under the low-threshold definition, the interaction is negative and significant ($-2.65$, $p < 0.05$), indicating that for extreme liars, positive sentiment does \textit{not} translate into higher contributions---consistent with cheap talk.
	...
	\begin{table}[H]
		...
		\input{issue_17_sentiment_liar_regression.tex}
	\end{table}
	...
	\begin{table}[H]
		...
		\small
		\input{contributions_summary.tex}
	\end{table}
