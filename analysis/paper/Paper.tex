...
\end{equation}
where $Contribution_{i,t}$ is the round level individual contribution, $promise_{i,t}$ is a binary variable indicating whether the individual made
a promise in that round, $sucker_{i,t}$ is a binary variable indicating whether the individual was a sucker in any previous round in the current segment, 
$treatment_{i}$ is a binary variable for treatment, $\alpha_r$ are round fixed effects and $\gamma_s$ are segment fixed effects.

\begin{table}
\input{contribution_regression.tex}
\caption{OLS results from specification \ref{eq:contribution_regression}. Standard errors clustered at the group level.}
\label{table:contribution_regression}
\end{table}

First, note that there is a significant and positive coefficient on the Treatment variable. Participants
contribute 1.62 more ECUs in expectation when they are unable to view the contributions of other group members. The
