...
| **IF / "Individual Feedback"** (treatment code `1` in raw data) | Players see **individual contributions** of each group member after each round, in addition to the group total. |
| **AF / "Aggregate Feedback"** (treatment code `2` in raw data) | Players see only the **group total** contribution after each round, not individual contributions. |
