"""
Tests for analyze_cumulative_liar_embeddings.py.

Tests cumulative-liar label computation from is_liar_20 boolean,
...
from analyze_cumulative_liar_embeddings import (
    compute_cumulative_liar_centroids,
    compute_cumulative_liar_labels,
    merge_cumulative_liar_labels,
    CLIAR_COL,
    STATE_LIAR,
    STATE_NON_LIAR,
    JOIN_KEYS,
...
def _make_state_df(rows_spec):
    """Build player-level DataFrame with is_liar_20 column.

    Args:
        rows_spec: list of (session, segment, round, group, label, is_liar)
    """
    rows = []
    for session, segment, rnd, group, label, is_liar in rows_spec:
        rows.append({
            'session_code': session, 'segment': segment,
            'round': rnd, 'group': group,
            'label': label, 'is_liar_20': is_liar,
        })
...
class TestComputeLiarLabels:
    """Tests for cumulative-liar label computation from boolean column."""

    def test_true_maps_to_liar(self):
        """is_liar_20=True -> 'liar' label."""
        df = _make_state_df([('s1', 'sg1', 2, 1, 'A', True)])
        result = compute_cumulative_liar_labels(df)
        assert result[CLIAR_COL].iloc[0] == STATE_LIAR

    def test_false_maps_to_non_liar(self):
        """is_liar_20=False -> 'non_liar' label."""
        df = _make_state_df([('s1', 'sg1', 2, 1, 'A', False)])
        result = compute_cumulative_liar_labels(df)
        assert result[CLIAR_COL].iloc[0] == STATE_NON_LIAR
    ...
    def test_mixed_labels(self):
        ...
        assert result[CLIAR_COL].tolist() == expected

    def test_output_columns(self):
        """Output should contain JOIN_KEYS and CLIAR_COL."""
        df = _make_state_df([('s1', 'sg1', 2, 1, 'A', True)])
        result = compute_cumulative_liar_labels(df)
        for key in JOIN_KEYS:
            assert key in result.columns
        assert CLIAR_COL in result.columns
    ...
    def test_all_same_label(self):
        ...
        assert (compute_cumulative_liar_labels(df_t)[CLIAR_COL] == STATE_LIAR).all()
        df_f = _make_state_df([
            ('s1', 'sg1', 2, 1, 'A', False),
            ('s1', 'sg1', 2, 1, 'E', False),
        ])
        assert (compute_cumulative_liar_labels(df_f)[CLIAR_COL] == STATE_NON_LIAR).all()
...
class TestMergeLiarLabels:
    """Tests for merge_cumulative_liar_labels."""

    def test_adds_liar_column(self):
        """Merged metadata should have CLIAR_COL."""
        meta = pd.DataFrame({
        ...
        liar_df = pd.DataFrame({
            'session_code': ['s1'], 'segment': ['sg1'],
            'round': [2], 'group': [1], 'label': ['A'],
            CLIAR_COL: [STATE_LIAR],
        })
        result = merge_cumulative_liar_labels(meta, liar_df)
        assert CLIAR_COL in result.columns
        assert result[CLIAR_COL].iloc[0] == STATE_LIAR

    def test_left_join_preserves_unmatched(self):
        """Unmatched rows should have NaN for CLIAR_COL."""
        meta = pd.DataFrame({
        ...
        result = merge_cumulative_liar_labels(meta, liar_df)
        assert len(result) == 2
        assert pd.isna(result[CLIAR_COL].iloc[1])
    ...
    def test_preserves_row_count(self):
        ...
        liar_df = pd.DataFrame({
            ...
            CLIAR_COL: [STATE_LIAR] * 3 + [STATE_NON_LIAR] * 3,
        })
        result = merge_cumulative_liar_labels(meta, liar_df)
        assert len(result) == 6
...
class TestCentroidComputation:
    """Tests for compute_cumulative_liar_centroids."""
    ...
    def test_centroids_are_mean_of_correct_subsets(self):
        ...
        l_c, nl_c = compute_cumulative_liar_centroids(emb, labels)
    ...
    def test_all_same_label_raises_error(self):
        ...
        with pytest.raises(ValueError, match="No"):
            compute_cumulative_liar_centroids(emb, labels)
...
class TestProjectionOutput:
    ...
    def test_build_output_has_cliar_column(self):
        """Output should include proj_cliar_pr_dir_{suffix} and CLIAR_COL."""
        meta = _make_metadata(4)
        projections = np.array([1.0, 2.0, -1.0, -2.0])
        result = _build_output(meta, projections, 'small', ID_COLS)
        assert 'proj_cliar_pr_dir_small' in result.columns
        assert CLIAR_COL in result.columns
        assert 'player_state' in result.columns

    def test_cross_level_output_columns(self):
        """Cross-level output should have all 4 proj_cliar columns."""
        n = 4
        ...
        for col in ['proj_cliar_msg_dir_small', 'proj_cliar_pr_dir_small',
...
class TestConstants:
    ...
    def test_cliar_col(self):
        """Cumulative-liar column name should match expected value."""
        assert CLIAR_COL == 'cumulative_liar_label'
