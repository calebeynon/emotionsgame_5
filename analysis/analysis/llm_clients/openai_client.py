"""
OpenAI API client for promise classification in chat messages.

Provides functions to classify messages as promises using GPT models
...
# =====
def classify_promise_openai(message: str, context: list[dict]) -> dict[str, Any]:
    """
    Classify if a message is a promise using OpenAI API.
...
def get_openai_cost_estimate(num_messages: int, avg_context_length: int) -> dict:
    """
...
# =====
def _call_api_with_retry(client: OpenAI, prompt: str) -> dict[str, Any]:
...
def _make_api_call(client: OpenAI, prompt: str) -> dict[str, Any]:
